...
/**
 * Factory for {@link SlowSynonymFilter} (only used with luceneMatchVersion <
 * 3.4)
 * 
 ...
 * </pre>
 */
final class ZKSlowSynonymFilterFactory extends TokenFilterFactory implements
		ResourceLoaderAware
{

	private final Logger				LOG		= LoggerFactory
														.getLogger(ZKSlowSynonymFilterFactory.class);
	...
	public ZKSlowSynonymFilterFactory(Map<String, String> args)
	{
		super(args);
		...
		LOG.info(
				"ZKSlowSynonymFilterFactory init args: synonyms={}, ignoreCase={}, expand={}",
				new Object[] { synonyms, ignoreCase, expand });
		...
		if (tf != null)
		{
			...
			// tokenizerFactory 初始化参数
			if (synonyms.contains("b2c"))
			{
				tokArgs.put("config", "/configs/b2c/");
			}
			else if (synonyms.contains("b2b"))
			{
				tokArgs.put("config", "/configs/b2b/");
			}
			else if (synonyms.contains("shop"))
			{
				tokArgs.put("config", "/configs/shop/");
			}
			else if (synonyms.contains("log"))
			{
				tokArgs.put("config", "/configs/log/");
			}
		}
		if (!args.isEmpty())
		{
	...
	public void inform(ResourceLoader loader) throws IOException
	{
		...
		SynonymLoderListener synonymListener = new SynonymLoderListener(
				synonyms, this, loader);
		synonymListener.executor(ZK.me().getClient()); // 启动DIC监听
		ZK.me().registerListener(synonymListener); // 加入ZK重连监听
	}

	protected void update(InputStream in, ResourceLoader loader)
	{
		try
		{
			Iterable<String> wlist = loadRules(in);
	 ...
	 */
	protected Iterable<String> loadRules(InputStream in) throws IOException
	{
		List<String> wlist = WordlistLoader.getLines(in, IOUtils.CHARSET_UTF_8);
		return wlist;
	}
